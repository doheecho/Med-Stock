...
SPLITS: dict[str, list[tuple[str, float]]] = {
    "005930": [("2018-05-04", 50)],       # 삼성전자 — 액면분할 50:1 (뉴스로 확정)
    "093230": [("2022-03-30", 0.1)],      # 이아이디 — 무상감자 10주→1주(2022-03-14 이사회, 감자비율 90%, 뉴스로 확정)
    # 아래 4개는 뉴스로 못 찾아서 "거래가 vs 그날 조정종가" 배율을 직접 계산해 넣음.
    # 표본이 여러 건이고 배율이 일관되게 모여서(변동폭 3~5% 이내) 신뢰도는 높지만,
    # 정확한 기준일은 모름 — 마지막으로 확인된 거래일 다음날로 잡음(이 종목들은
    # 전부 현재 미보유라 오늘 평가엔 영향 없고, 그 구간 과거 곡선에만 영향).
    "035760": [("2018-07-02", 0.4181)],  # CJ E&M→CJ ENM(구 CJ오쇼핑과 합병) — 30건 평균 배율, 표본 가장 많고 확실
    "044380": [("2015-12-01", 0.1097)],  # 주연테크 — 5건 평균 배율(0.1055~0.114, 거의 9~9.5:1)
    "061040": [("2019-08-01", 0.2060)],  # 알에프텍 — 3건 평균 배율(0.19~0.22, 약 5:1)
    "017670": [("2018-02-09", 6.0275)],  # SK텔레콤 — 2018-02-08 하루치 매수·매도 2건뿐이지만 배율이
                                          # 6.01/6.05 로 거의 일치해 신뢰도 있음(2025년 거래는 배율 1.0 정상)
    "036630": [("2015-10-30", 0.098)],   # 세종텔레콤 — 2015-10-20 매수 4건 배율 0.089~0.102 로 일관(평균
                                          # 사용). 같은 배치의 2015-10-29 매도 1건은 0.186 로 좀 떨어져 있어
                                          # 매수 쪽 표본만 사용 — 정확한 감자 시점은 못 찾음.
}
# 참고: 홈캐스트(064240)도 확인해봤지만 거래가/조정종가 배율이 전부 0.8~1.2 범위로
# 정상적인 변동성 수준(2016-09 에 실제로 며칠 새 20% 이상 출렁인 종목) — 스케일
# 불일치가 아니라서 SPLITS 에 안 넣음. "차트가 이상하다"는 느낌은 액면분할이
# 아니라 그냥 그 시기 실제 변동성이 컸던 것으로 보임.
